  ...
  public async connect(connectionString?: string, options: ConnectionOptions = {}): Promise<void> {
    try {
      ...
      if (poolCache.has(connString)) {
        ...
        // Create a new pool
        let sslConfig = options.ssl;
        const sslCertPath = process.env.POSTGRES_SSL_CERT_PATH;
        if (sslCertPath) {
          try {
            const ca = fs.readFileSync(sslCertPath, 'utf8');
            sslConfig = { ca, rejectUnauthorized: true } as any;
          } catch (err) {
            throw new Error(`Failed to read SSL certificate from ${sslCertPath}: ${err instanceof Error ? err.message : String(err)}`);
          }
        }
